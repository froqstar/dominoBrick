...
- Symbol timing is early-late on three windows with hold-on-clean-lock (`co > 10` + small imbalance → `corr = 0`). During steady tones timing is unobservable; any voter that corrects every symbol random-walks off the grid.
